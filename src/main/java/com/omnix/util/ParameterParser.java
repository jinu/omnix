...
public class ParameterParser {
	...
	/**
	 * 파라미터를 Date 형으로 읽는다. "yyyy-MM-dd" 형식이다. 파라미터가 없거나 형식이 맞지 않는 경우 현재 시간을 사용한다.
	 */
	...
	public Date getDateSearchDate(int addHour) {
		...
		if (StringUtils.isEmpty(date1)) {
			return DateUtils.addMonths(new Date(), addHour);
		}
	...
	public Date getDateStart(int day) {
		...
		if (StringUtils.isEmpty(date1)) {
			Date temp = DateUtils.addDays(new Date(), day);
	...
	public Date getDateStartDate(int addHour) {
		...
		if (StringUtils.isEmpty(date1) || StringUtils.isEmpty(date2)) {
			return DateUtils.addHours(new Date(), addHour);
		}
	...
	public Date getDateStartDateMonth(int addMonth) {
		...
		if (StringUtils.isEmpty(date1) || StringUtils.isEmpty(date2)) {
			return DateUtils.addMonths(new Date(), addMonth);
		}
	...
	public String[] getArray(String name) {
		...
		for (int i = 0; i < tempArray.length; i++) {
			if (null != tempArray[i] && 0 != tempArray[i].length()) {
				list.add(StringUtils.trim(tempArray[i]));
			}
		}
	...
	public List<String> getList(String name) {
		return Arrays.stream(getArray(name)).collect(Collectors.toList());
	}
	...
	public List<Integer> getListIntArray(String name) {
		String[] tempArray = req.getParameterValues(name);
		if (null == tempArray) {
	...
	public String toString() {
		...
		while (params.hasMoreElements()) {
			...
			sbf.append(paramName).append("=").append(req.getParameter(paramName)).append(", ");
		}
		return sbf.toString();
	}
	...
	public String getParamString() {
		...
		return StringUtils.substring(stringBuilder.toString(), 0, stringBuilder.toString().length() - 1);
	}
